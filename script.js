...
window.onload = function() {
  ...
  document
    ...
    .addEventListener('submit', adicionarOuAlterar);
...
function adicionarOuAlterar(e) {
  ...
  var p = {
    ...
    nasc: new Date(
      document.getElementById('dtpDataNascimento').value.replace('-', '/')
    ),
  ...
  if (idAlterar == null) adicionar(p);
  else if (idAlterar > 0) alterar(p);
  else alert('Ação desconhecida');
...
function adicionar(p) {
    ...
    if (pessoas.length > 0)
      idValido = (function obterIdValido() {
        ...
        for (var i = 0; i < pessoas.length; i++)
          if (pessoas[i].Id != i + 1) return i + 1;
  ...
  var pessoa = {
    ...
    Sexo: p.sexo,
    Nivel: p.nivel.options[nivel.selectedIndex].value,
...
function alterar(p) {
  ...
  for (var i = 0; i < pessoas.length; i++) {
    if (pessoas[i].Id == idAlterar) {
      ...
      pessoas[i].Nivel = p.nivel.options[nivel.selectedIndex].value;
...
function excluir(cod) {
  ...
  for (var i = 0; i < pessoas.length; i++)
    if (pessoas[i].Id == cod) pessoas.splice(i, 1);

  localStorage.setItem('value', JSON.stringify(pessoas));
  listar();

  //se nao possuir mais nenhum registro, limpar o storage
  if (pessoas.length == 0) window.localStorage.removeItem('value');
}

function listar() {
  //se nao possuir nenhum local storage, nao fazer nada
  if (localStorage.getItem('value') === null) return;
  ...
  for (var i = 0; i < pessoas.length; i++) {
    ...
    tbody.innerHTML +=
      ...
      id +
      '\')">Excluir</button></td>' +
      '<td><button onclick="prepararAlterar(\'' +
      id +
      '\')">Alterar</button></td>' +
      '</tr>';
  }
